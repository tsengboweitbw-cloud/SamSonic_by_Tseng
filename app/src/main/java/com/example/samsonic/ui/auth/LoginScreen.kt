...
fun LoginScreen(
    ...
    fun connect() {
        ...
        errorMessage = null
        scope.launch {
            val result = repository.connect(serverUrl, username, password)
            isConnecting = false
            result.fold(
                onSuccess = { onConnected() },
                onFailure = { errorMessage = it.message ?: "Couldn't connect to that server" },
            )
        }
    }
    ...
    Column(
        ...
        Text(text = "SamSonic", style = MaterialTheme.typography.headlineMedium)
